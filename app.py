...
if st.button("🔥 Ejecutar Benchmark Estratégico", type="primary"):
    if not marca or not sector or not pais or not ciudad or not producto:
        ...
        status_box.info(f"🔍 Fase 1/3: Rastreando líderes de mercado e instituciones en {ciudad}, {pais}...")
        
        sector_corto = sector.split(",")[0].split("/")[0].strip()
        producto_corto = producto.split(",")[0].split(".")[0].strip()[:30]
        
        # === 🌐 INTERRUPTOR GEOGRÁFICO (Configuración de Base de Datos por País) ===
        pais_norm = pais.lower().strip()
        if "colombia" in pais_norm:
            site_filter = "(site:rues.org.co OR site:supersociedades.gov.co OR site:confecamaras.org.co OR site:clutch.co)"
        elif "mexico" in pais_norm or "méxico" in pais_norm:
            site_filter = "(site:siem.economia.gob.mx OR site:clutch.co OR site:goodfirms.co)"
        elif "españa" in pais_norm or "espana" in pais_norm:
            site_filter = "(site:rmc.es OR site:northdata.com OR site:clutch.co)"
        else:
            site_filter = "(site:clutch.co OR site:trustpilot.com OR site:g2.com OR site:capterra.co OR site:goodfirms.co OR site:opencorporates.com OR site:annualreports.com)"

        # Búsquedas Web Optimizadas con Interruptor Geográfico
        ...
        registros_web = buscar_urls_reales(f"empresas {sector_corto} {pais} {site_filter}", max_results=8)
        inter_web = buscar_urls_reales(f"top rated global companies agencies {sector_corto} {producto_corto}", max_results=8)
        
        # 🎯 BÚSQUEDA DE INSPIRACIÓN HIPER-ESPECÍFICA (Cruzando Sector + Producto)
        insp_web = buscar_urls_reales(f"{sector_corto} {producto_corto} branding identity design (site:awwwards.com OR site:thedieline.com OR site:cosmos.so OR site:reallygoodemails.com OR site:brandarchive.xyz OR site:itsnicethat.com OR site:fastcompany.com OR site:underconsideration.com OR site:oneclub.org)", max_results=6)
        ...
        todos_los_hallazgos = fijos_lista + locales_web + nacionales_web + registros_web + inter_web + insp_web
        ...
        if len(hallazgos_unicos) >= 3:
            status_box.info("🧠 Evaluando marcas con filtro de élite y validación por país...")
