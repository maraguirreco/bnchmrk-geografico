...
def parsear_json_llm(texto):
    ...
    # Limpieza simple y segura de bloques markdown
    ...
    try:
        return json.loads(raw_json_clean)
    except Exception:
        pass
        
    try:
        return ast.literal_eval(raw_json_clean)
    except Exception:
        pass
        ...
        if results:
            for r in results:
                ...
                if not url or any(bad in url for bad in bad_domains):
                    continue
                if excluir_clean and (excluir_clean in title_clean or excluir_clean in url):
                    continue
                ...
                if len(urls_validas) >= max_results:
                    break
    except Exception:
        pass
    return urls_validas
...
st.markdown("Genera estudios de competencia y expórtalos directamente a **Miro**.")
